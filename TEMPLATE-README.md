...
|---|---|---|
| `LLM Wiki 101` | `Kubernetes 101` | titles, headers, OG tags |
| `LLM Wiki เจาะลึกเชิงเทคนิค` | `Kubernetes เจาะลึกเชิงเทคนิค` | Thai pages, subtitles |
| `llm-wiki-101` | `kubernetes-101` | repo references in URLs |
| `Build persistent LLM-maintained knowledge bases — Karpathy's "LLM Wiki" pattern, distilled` | `Master Kubernetes the open way` | hero, OG description |
| `สร้าง knowledge base ที่ LLM ดูแลให้ — pattern ของ Karpathy ปรับใช้จริง` | `เรียนรู้ Kubernetes แบบเปิด` | Thai hero |
| `Supachai Jaturaprom` | `Supachai Jaturaprom` | footer, OG |
| `supachai-j` | `supachai-j` | GitHub URLs |
| `https://github.com/supachai-j/llm-wiki-101` | `https://github.com/supachai-j/kubernetes-101` | links |
| `https://supachai-j.github.io/llm-wiki-101/` | `https://supachai-j.github.io/kubernetes-101/` | OG, canonical |
| `LLM Wiki — a design pattern for persistent, LLM-maintained knowledge bases. Three-layer architecture (raw sources / wiki / schema), three core operations (ingest / query / lint), and the lifecycle that turns RAG-style "re-derive every time" into a compounding knowledge artifact. Two tracks: engineers building the systems, and researchers using them for personal knowledge management.` | `Kubernetes — what it is, ...` | wiki SCHEMA, descriptions |
